...
- TRIM shows the same selected sample plus deterministic window metadata: start frame, end frame, length frames, and source pad. In the original catalog slice this was derived placeholder metadata; the later trim-window slice makes it editable.
...
- Project snapshots persist `machine.selected_sample_id` with `serde(default)` compatibility. Missing selected sample ids normalize to the first available sample on restore, or `None` if the catalog is empty. Present unknown ids are rejected by the later trim-window validation contract so stale selections cannot silently point at the wrong sample.
...
- Current implementation derives initial frame lengths from the source pad address for deterministic testability only.
